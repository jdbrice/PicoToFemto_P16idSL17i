...
public:
  ...
  double globalDCA( double B, StThreeVectorF vtx );
...
/*************************** DANIEL ADDED ********************************/
inline double StPicoTrack::globalDCA( double B, StThreeVectorF vtx ){
    
    StPhysicalHelixD gHelix = helix( B );
    gHelix.moveOrigin( gHelix.pathLength(vtx) );

    StThreeVectorF origin = gHelix.origin();
    StThreeVectorF HMom   = gHelix.momentum( B * kilogauss); // taken from PicoEvent
